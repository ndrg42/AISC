...
def main():
    ...
    X_hosono = list(np.load(home_path + "/data/processed/hosono_dataset.npy"))
    Y_hosono = pd.read_csv(home_path + '/data/external/hosono_ready.csv')['critical_temp']

    Y_hosono_pred = tf.reshape(model.predict(X_hosono), shape=(-1,))
    mae = tf.keras.metrics.mean_absolute_error(Y_hosono_pred, Y_hosono)
    mse = tf.keras.metrics.mean_squared_error(Y_hosono_pred, Y_hosono)
    print(f"MAE : {mae}\nMSE : {mse}\nRMSE : {tf.math.sqrt(mse)}")
